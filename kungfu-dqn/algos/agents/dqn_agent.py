...
class DQNAgent():
    def __init__(self, input_shape, action_size, seed, device, buffer_size, batch_size, gamma, lr, tau,
                 update_every, replay_after, model, train, load_epoch, model_dir, momentum):
        ...
        self.optimizer = optim.RMSprop(self.policy_net.parameters(), lr=self.lr, momentum=self.momentum)

        # load the agent for test purpose
        if not self.train:
            print('Load the model')
            policy_path = self.model_dir[0]+'_{}.pth'.format(self.load_epoch)
            target_path = self.model_dir[1]+'_{}.pth'.format(self.load_epoch)
